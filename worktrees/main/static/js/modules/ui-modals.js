...
export function initializeAPToggle() {
    ...
    const modeEntries = [
        { id: 'ap-mode-manual', label: 'Manual' },
        { id: 'ap-mode-hold', label: 'Hold' },
        { id: 'ap-mode-auto', label: 'Auto' },
        { id: 'ap-mode-loiter', label: 'Loiter' },
        { id: 'ap-mode-acro', label: 'Acro' },
        { id: 'ap-mode-rtl', label: 'RTL' }
    ];

    const getCurrentMode = () => apToggleBtn.dataset.currentMode || apToggleBtn.textContent.trim();

    const highlightSelection = (modeName) => {
        modeEntries.forEach(({ id }) => {
    ...
    const sendApMode = (modeName) => {
        if (window.ws && window.ws.readyState === WebSocket.OPEN) {
            const message = { type: 'ap.set_mode', mode: modeName };
            window.ws.send(JSON.stringify(message));
            console.log('Sent AP mode:', modeName);
        }
    };

    const applyMode = (modeName) => {
        apToggleBtn.textContent = modeName;
        apToggleBtn.dataset.currentMode = modeName;
        highlightSelection(modeName);
        sendApMode(modeName);
    };
    ...
    const openModal = () => {
        ...
        highlightSelection(getCurrentMode());
    };
...
export function initializeHelmToggle() {
    ...
    const helmOptions = [
        { id: 'helm-option-main', label: 'Main' },
        { id: 'helm-option-aux', label: 'AUX' },
        { id: 'helm-option-robot', label: 'Robot' }
    ];

    const setVisualState = (selectedLabel) => {
        helmOptions.forEach(({ id }) => {
            const optionEl = document.getElementById(id);
            if (!optionEl) return;
            const valueEl = optionEl.querySelector('.helm-display-value');
            const isActive = valueEl?.textContent.trim() === selectedLabel;
            if (valueEl) {
                valueEl.style.color = isActive ? '#ffffff' : '#757575';
                valueEl.style.fontWeight = isActive ? '600' : '500';
            }
        });
    };

    const sendHelmSelection = (selectedHelm) => {
        if (window.ws && window.ws.readyState === WebSocket.OPEN) {
            const message = { type: 'helm.set', helm: selectedHelm };
            window.ws.send(JSON.stringify(message));
            console.log('Sent Helm selection:', selectedHelm);
        }
    };

    const applyHelmSelection = (label) => {
        helmToggleBtn.textContent = label;
        helmToggleBtn.dataset.selectedHelm = label;
        setVisualState(label);
        sendHelmSelection(label);
    };
    ...
    const openModal = () => {
        ...
        const current = helmToggleBtn.dataset.selectedHelm || helmToggleBtn.textContent.trim();
    ...
    helmOptions.forEach(({ id, label }) => {
        const optionEl = document.getElementById(id);
        if (!optionEl) return;
        optionEl.addEventListener('click', () => {
            applyHelmSelection(label);
            closeModal();
        });
    });
    ...
    const initialLabel = helmOptions.find(opt => opt.label === helmToggleBtn.textContent.trim())?.label || helmOptions[0].label;
    helmToggleBtn.textContent = initialLabel;
    helmToggleBtn.dataset.selectedHelm = initialLabel;
    setVisualState(initialLabel);
}
